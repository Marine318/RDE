...
def get_loss(model, data_loader):
    ...
    # 添加防护避免除以0
    A_range = lossA.max() - lossA.min()
    B_range = lossB.max() - lossB.min()
    
    if A_range == 0:
        losses_A = torch.zeros_like(lossA)
    else:
        losses_A = (lossA-lossA.min())/A_range
        
    if B_range == 0:
        losses_B = torch.zeros_like(lossB)
    else:
        losses_B = (lossB-lossB.min())/B_range
    ...
    # 将NaN替换为该批次的平均值或0
    mask_A = np.isnan(input_loss_A_np)
    mask_B = np.isnan(input_loss_B_np)
    
    if mask_A.any():
        mean_A = np.nanmean(input_loss_A_np)
        input_loss_A_np[mask_A] = 0.0 if np.isnan(mean_A) else mean_A
        logger.warning(f"Found {mask_A.sum()} NaN values in loss A, replaced with {0.0 if np.isnan(mean_A) else mean_A}")
        
    if mask_B.any():
        mean_B = np.nanmean(input_loss_B_np)
        input_loss_B_np[mask_B] = 0.0 if np.isnan(mean_B) else mean_B
        logger.warning(f"Found {mask_B.sum()} NaN values in loss B, replaced with {0.0 if np.isnan(mean_B) else mean_B}")
